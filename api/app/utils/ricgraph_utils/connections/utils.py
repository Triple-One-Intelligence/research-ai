...
def run_query(query: str, session: Any | None = None, **params: Any) -> list[dict[str, Any]]:
    """Execute a single graph query with unified error handling."""
    try:
        return database_utils.execute_cypher(query, session=session, **params)
...
def run_type_query(
    ...
    *,
    ...
    if entity_type == "person":
        return run_query(person_query, session=session, **person_params)
    return run_query(organization_query, session=session, **organization_params)
...
def person_connections(
    ...
    with database_utils.get_graph().session() as session:
        return {
            "collaborators": get_collaborators(
                entity_id, "person", max_collaborators, session=session
            ),
            "publications": get_publications(
                entity_id, "person", max_publications, session=session
            ),
            "organizations": get_organizations(
                entity_id, "person", max_organizations, session=session
            ),
            "members": [],
        }
